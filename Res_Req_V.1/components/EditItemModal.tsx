...
interface EditItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: Resource, quantity: number) => void;
  item: Resource | null;
  lang: Language;
...
const EditItemModal: React.FC<EditItemModalProps> = ({ isOpen, onClose, onSave, item, lang, initialQuantity }) => {
  ...
  useEffect(() => {
    if (item) {
      setItemEnglish(item.item_english);
      setItemArabic(item.item_arabic);
      setUnit(item.unit);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (item && itemEnglish.trim() && itemArabic.trim() && unit.trim()) {
      onSave({ ...item, item_english: itemEnglish, item_arabic: itemArabic, unit }, quantity);
      onClose();
    }
  };
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-secondary-gray-dark mb-1">{t.quantity}</label>
                <input
                  type="number"
                  min="0"
                  value={quantity}
            ...
            <div className="mt-6 flex justify-end space-x-3 rtl:space-x-reverse">
              ...
              <button
                type="submit"
                className="px-4 py-2 bg-accent text-white font-semibold rounded-md hover:bg-accent-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent transition"
              >
                {t.save}
              </button>
            </div>
